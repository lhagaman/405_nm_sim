...
allphotons_WavelengthHist = RCT.quickTH1D("allphotons_Wavelength", "allphoton_Wavelength", 750, 50, 700,
                                          "Wavelength [nm]", "Entries")
UVphotons_WavelengthHist = RCT.quickTH1D("UVphotons_Wavelength", "UVphotons_Wavelength", 750, 50, 700,
                                         "Wavelength [nm]", "Entries")
visPhotons_WavelengthHist = RCT.quickTH1D("visphotons_Wavelength", "visphotons_Wavelength", int(750. / 5), 50, 700,
                                          "Wavelength [nm]", "Entries")

# check conservation of energy
excessEnergyHist = RCT.quickTH1D("excessEnergyAfterWLS", "excessEnergyAfterWLS", int(25 / 0.1), -5, 20, "Energy [ev]",
                                 "Entries")
...
UVPenetrationDepth = RCT.quickTH1D("UVPenetrationDepth", "UVPenetrationDepth", 100, 0.0,
                                   (max(TPBboundaryValues) - min(TPBboundaryValues)) * 10 ** 3,
                                   "UV TPB Penetration Depth", "Entries")
UVDotProdAtUpstreamTracking = RCT.quickTH1D("UVDotProdAtUpstreamTracking", "UVDotProdAtUpstreamTracking", 120, -1.1,
                                            1.1, "UV Termination X Coord", "Entries")
...
VisLocalReemissionDotProd = RCT.quickTH1D("VisLocalReemissionDotProd", "VisLocalReemissionDotProd", 120, -1.1, 1.1,
                                          "Vis Local Reemission Dist Dot X", "Entries")
VisPhotonCreationX = RCT.quickTH1D("VisCreationCoordX", "VisCreationCoordX", 100, 0.0,
                                   (max(TPBboundaryValues) - min(TPBboundaryValues)) * 10 ** 3, "Vis Creation X Coord",
                                   "Entries")
VisPhotonAbsX = RCT.quickTH1D("VisAbsCoordX", "VisAbsCoordX", 100, 0.0,
                              (max(TPBboundaryValues) - min(TPBboundaryValues)) * 10 ** 3, "Vis Abs X Coord", "Entries")
VisPhotonAbsLength = RCT.quickTH1D("VisAbsLength", "VisAbsLength", 100, 0.0, 20., "Vis Abs Length", "Entries")
VisUpstreamDiskDotProd = RCT.quickTH1D("VisUpstreamDiskDotProd", "VisUpstreamDiskDotProd", 120, -1.1, 1.1,
                                       "Vis Upstream Disk Flux Dot X", "Entries")
VisDownstreamDiskDotProd = RCT.quickTH1D("VisDownstreamDiskDotProd", "VisDownstreamDiskDotProd", 120, -1.1, 1.1,
                                         "Vis Downstream Disk Flux Dot X", "Entries")
VisPhotonsCreatedByUVPhoton = RCT.quickTH1D("VisPhotonsCreatedByUVPhoton", "VisPhotonsCreatedByUVPhoton", 10, 0, 10,
                                            "Number of Visible Photons Produced By UV", "Entries")
...
allTheHists += [VisTermX, VisTermY, VisTermZ, VisLocalReemissionDotProd, VisPhotonCreationX, VisPhotonAbsX,
                VisPhotonAbsLength, VisUpstreamDiskDotProd, VisDownstreamDiskDotProd, VisPhotonsCreatedByUVPhoton]
...
for iEntry, anEntry in enumerate(fileIterator):
    ...
    if 'tpbVolume' in tempUVTrackVolumes and 'upstreamTrackingDisk' in tempUVTrackVolumes:
        ...
        elif lastStepVolume != 'tpbVolume' and numTracks == 1 and lastStepXDot > 0.0 and lastStepEndpoint.x() > max(
                TPBboundaryValues):
    ...
    for iTrack in range(numTracks):
        ...
        if firstVisStepEndpoint.x() == UVEndpoint.x() and firstVisStepEndpoint.y() == UVEndpoint.y() and firstVisStepEndpoint.z() == UVEndpoint.z() and VisWavelength > 350.:
            ...
            VisParentWasntUV = False
        elif VisWavelength > 350 and (
                    firstVisStepEndpoint.x() == UVEndpoint.x() and firstVisStepEndpoint.y() == UVEndpoint.y() and firstVisStepEndpoint.z() == UVEndpoint.z()) == False:
...
jsonStructOut = {'UVStepVolumes': UVStepVolumes, 'UVStepProcesses': UVStepProcesses,
                 'UVStepEndVolumes': UVStepEndVolumes, 'UVStepEndProcess': UVStepEndProcess,
                 'VisStepVolumes': VisStepVolumes, 'VisStepProcesses': VisStepProcesses,
                 'VisStepEndVolumes': VisStepEndVolumes, 'VisStepEndProcess': VisStepEndProcess,
                 'vis_detect_count': vis_detect_count, 'vis_total_count': vis_total_count,
                 'UVPhotonStopInTPBAnNoVis': UVPhotonStopInTPBAnNoVis,
                 'UVPhotonStopInTPBAndVis': UVPhotonStopInTPBAndVis, 'UVPhotonPassThroughTPB': UVPhotonPassThroughTPB,
                 'UVPhotonsPassThroughUpstream': UVPhotonsPassThroughUpstream,
                 'UVPhotonsReflectedOffUpstreamTPBSurf': UVPhotonsReflectedOffUpstreamTPBSurf,
                 'VisPhotonsNotCreatedByParentUV': VisPhotonsNotCreatedByParentUV,
                 'VisPhotonsCreatedByParentUV': VisPhotonsCreatedByParentUV,
                 'VisPhotonsPassingUpstreamDisk': VisPhotonsPassingUpstreamDisk,
                 'VisPhotonsPassingDownstreamDisk': VisPhotonsPassingDownstreamDisk,
                 'VisPhotonsAbsInTPB': VisPhotonsAbsInTPB, 'VisPhotonsAbsInAcrylic': VisPhotonsAbsInAcrylic,
                 'VisPhotonDetectedFromVis': VisPhotonDetectedFromVis,
                 'VisPhotonDetectedFromUV': VisPhotonDetectedFromUV,
                 'VisPhotonsNotCreatedByParentUV': VisPhotonsNotCreatedByParentUV}
